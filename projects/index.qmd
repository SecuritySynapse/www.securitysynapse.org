...
out these projects to learn more about how to apply what you are learning in
the field of computer security! The security survey projects invite you to
complete work in fields like fuzzing and cryptography. The long-term security
summit project offers the scaffolding for learners to complete independent
research and development in the field of computer security. Each project
explores a specific topic in connection with content from extra online
resources or one of these online textbooks:
...
[CSP](https://web.njit.edu/~rt494/security/) explains that a server
demonstrates *availability* if it is up and running for valid users. Yet, if a
server is running a process that contains a defect that results in a system
crash, then this is a direct threat to its availability. A program that crashes
when it is given erroneous inputs is not robust. However, since the programs
that run on servers often have complex input spaces and implementations, it is
often difficult to quickly find these availability threats without appropriate
...
certain types of JSON inputs, submit those inputs to an in-memory server,
observe the output and behavior of the server, record important details and any
crashes, and minimize the JSON inputs that cause a failure. As you complete the
project, please be mindful that it places certain restrictions on the types of
JSON inputs your `serverfuzzer` should generate!

#### Resources

To learn more about fuzzing and to gather inspiration for your own
`serverfuzzer` tool, please consult the following resources about building
fuzzers:
...
- [AFL++: A fuzzing framework built as a descendant of the original AFL](https://aflplus.plus/)
...
Questions? Share them in the [Security Synapse Discord
Server](https://discord.gg/juUXz7d4Jh). Want to take this project to the next
level? Consider meeting with the instructor during office hours!
...
variety of different cryptography algorithms and explain how they may
be cracked.
...
ciphers. This program should randomly generate data and then measure and report
the time and space overheads arising from the use of, at minimum, the `Fernet`
and `rFernet` ciphers.

#### Resources

To learn more about cryptography and to gather inspiration for your own
...
To learn more about network monitoring and to gather inspiration for
your own implementation of the `networkmonitor`, please consult the
following technical resources about network communication with either
sockets or XML-RPC in Python, the TCP/IP protocol and the layout of
their headers and payloads, and various network monitoring techniques:
...
Questions? Share them in the [Security Synapse Discord
Server](https://discord.gg/juUXz7d4Jh).
...
program trace can help a security engineer to discern when it is infected
with malware. Program tracing can also be used to analyze the behavior of
malware to understand how it operates and to aid in its detection.
...
format. The `programtracer` tool should have these features:
  ...
  the similarities and differences between them. Examples of trace
...
behavior. To learn more about program tracing and to gather inspiration for
your own implementation of the `programtracer`, please consult the
following technical resources about techniques for tracing Python programs
for program understanding, debugging, and malware analysis:
...
complete an independent research project in the field of computer security.
  ...
  - {{< iconify fa6-solid diagram-project size=18px >}} **Tasks for the Lightning Talk**
