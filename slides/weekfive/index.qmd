...
- Never stores its own code as a file on the disk

- Runs that code in memory, inside another process

- **Primary goal**: hide from file-based malware scanners
...
- A scanner looks for known bad code inside files
- Fileless malware never gives it a file to inspect
- Instead, the code travels as ordinary text
- `base64` rewrites code as text that looks like noise
- Anyone can reverse `base64`, so it obscures attack

:::

::: {.fragment .fade .boxed-content style="padding-top: .5em; font-size: 0.9em;"}

**Key idea**: From the perspective of Python programming, `print('hi')` and
`cHJpbnQoJ2hpJyk=` are the same program. Try this out! Now, do a deep-dive
on `base64` encoding in Python!
...
# the payload is a program, stored as plain text
payload = "print('payload ran without a file')"
hidden = base64.b64encode(payload.encode()).decode()
print(f"hidden: {hidden}")

# rebuild the program and run it, writing no file
...
- `b64encode` disguises it, and `b64decode` restores it

- `exec` runs the restored program without a file
...
- The file holds text that looks like noise

- The program that runs is ordinary Python

- Records of the hidden code live in memory, not on disk

- Payload leaves nothing on disk, so memory is the evidence
...
- Cryptojacking can be spread via:
...
- Mining adds the next block to a blockchain
- A block is accepted when its digest has leading zeros
- The miner picks a "free number" called the `nonce`
  - A *nonce* is a number used once
- Each new `nonce` gives a completely different digest
- So the miner keeps guessing until one digest passes
- The miner who finds it earns the block reward
- Use the victim's computer to find a passing nonce!
...
- SHA-256 is one-way, so a digest hides its `nonce`
- There is no shortcut, so the miner must guess
- Checking one guess costs a single hash
- So any peer can verify a `nonce` almost instantly
- The guessing is paid for in electricity
...
files? What are the trade-offs of this approach? Why? Now, investigate
further!**
...
- A guess wins about once in 4,096 at three zeros

- At four zeros a win takes 65,536 guesses, 16x more

- The runs above are single, lucky draws

- Mining costs are paid by the computer's owner!
