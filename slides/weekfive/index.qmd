...
# *Probing Questions*: have you ever encountered malware? What happened? How did you resolve it? Lessons?
...
- Malware does not always need elevated privileges to run
- *Goal*: privilege escalation, perhaps without user interaction
- What is a zero-day exploit?
  - A vulnerability unknown to the vendor or defenders
  - Often has no patch available when first exploited
  - Can support malware delivery, code execution, privilege escalation,
...
# Wow, A Browser Vulnerability Involving Malicious JavaScript

::: fragment

[Read the article about CVE-2024-45489 by eva!](https://kibty.town/blog/arc/)
...
- Different types and behaviors of malware
- Some malware span multiple categories
- Malicious code can appear in source code and binaries
- Malicious code may be embedded in documents or scripts
...
- Some worms can spread without further user interaction
...
- Modifies a program and inserts its own code

- Viruses often require user action to begin spreading

- **Defining behavior**: replicate by infecting other programs
...
- Disguises itself as a legitimate file or program

- Trojan horses often require user action to install or run

- **Possible goal**: provide access, steal data, or deliver other types of
malware (e.g., ransomware, spyware, or worms)
...
- Encrypts a victim's files after gaining access

- Demands payment to decrypt the files or restore access
...
::: {.fragment style="margin-top: 0.00em; font-size: 0.95em;"}

- May avoid storing its main payload as a traditional executable file,
  which can sometimes evade detection by antivirus software

- May run code in memory or through a trusted interpreter

- **Possible benefit**: reduce reliance on file-based detection

- Fileless malware often relies on:
  - Code that arrives as text and is rebuilt in memory
  - Tools that are already installed on the machine
  - Script engines such as Python, Bash, or PowerShell

:::

::: {.fragment style="font-size: 0.95em; margin-top: -0.5em;"}

{{< iconify fa6-solid map >}} **What evidence of malware remains when no file
is written?**

:::

## Hiding Code as Text with `base64`

::: {.fragment style="margin-top: 0.00em; font-size: 0.95em;"}

- A simple scanner may search for recognizable text in files
- An attacker may send a payload or command as ordinary text
- `base64` encodes the bytes into a less recognizable text form
- Base64 is encoding, not encryption, and anyone can decode it
- A loader must decode the content before it can execute or use it

:::

::: {.fragment .fade .boxed-content style="padding-top: .5em; font-size: 0.8em;"}

**Key idea**: `cHJpbnQoJ2hpJyk=` is a Base64-encoded representation of
`print('hi')`; Python must decode it before it can execute the source. Try
this out, then consider how a detector could decode suspicious text!
...
# a harmless payload is stored as plain text
payload = "print('payload ran without a separate payload file')"
hidden = base64.b64encode(payload.encode()).decode()
print(f"hidden: {hidden}")

# decode and run it without creating a separate payload file
source = base64.b64decode(hidden).decode()
exec(source)
```

::: {.fragment .fade style="padding-top: .2em; font-size: 0.8em;"}

- The payload is a program stored as ordinary text
- `b64encode` changes its representation; `b64decode` restores it
- `exec` runs the restored program without a separate payload file
...
```{python}
print(f"on disk:   {hidden}")
print(f"in memory: {source}")
```

::: {.fragment style="margin-top: 0.00em; font-size: 0.9em;"}

- The encoded text may be less recognizable to a simple string search
- The program that runs is ordinary Python after decoding
- This cell creates no separate payload file
- The decoded source remains available in the process's memory

:::

::: {.fragment style="font-size: 0.95em; margin-top: 0.1em;"}

{{< iconify fa6-solid people-line >}} **How would you find the active payload in
memory? How is this more difficult than detecting file-based malware? Ideas?**

:::

## Key Insights About Malware

::: incremental

- A variety of forms (e.g., ransomware, spyware, worms)
- Actions may follow once malware is running with access
...
::: {.fragment .boxed-content style="padding-top: .5em; font-size: 0.7em;"}

- Which common practices promote the spread of malware?
- Which language or runtime choices affect detection?
- Trade-offs in implementing and defending against malware?
...
- Mining must be hard to do and easy to check
- The miner who seals a block earns the reward
- A cryptojacker makes a victim's machine do that work

:::

::: {.fragment style="font-size: 0.95em; margin-top: -0.1em;"}

{{< iconify fa6-solid people-line >}} **Wait, who pays for the mining? How is
this an attack?**
...
- Steals processor cycles and electricity instead of files

- Mines cryptocurrency in the background

- **Primary goal**: earn coins with someone else's hardware

- Cryptojacking can be spread through:
  - Malicious web pages and browser scripts
  - Trojanized downloads and browser extensions
  - Exploited servers and Internet of Things (IoT) devices

:::

::: {.fragment style="font-size: 0.95em; margin-top: -0.1em;"}

{{< iconify fa6-solid map >}} **Can you find an example of cryptojacking?**

:::

## Sealing a Block with Proof of Work

::: {.fragment style="margin-top: 0.00em; font-size: 0.9em;"}

- In proof-of-work systems, mining proposes the next block
- In this toy model, a block wins when its digest has leading zeros
- The miner picks a "free number" called the `nonce`
  - A *nonce* is a value varied to produce a new candidate digest
- Each new `nonce` gives a different candidate digest
- So the miner keeps guessing until one digest passes
- A successful miner may receive the block reward
- In cryptojacking, the victim's computer performs this work

:::

::: {.fragment style="font-size: 0.95em; margin-top: -0.1em;"}

{{< iconify fa6-solid people-line >}} **Again, can you explain why this is considered an attack?**

:::

## Why Guessing Is the Work

::: {.fragment style="margin-top: 0.00em; font-size: 0.9em;"}

- A cryptographic hash makes the winning `nonce` hard to predict
- In the toy model, the miner tests candidates until one succeeds
- Checking one guess costs a hash computation
- So any peer can verify a `nonce` almost instantly
- The guessing is paid for in electricity at no cost to attacker
- A cryptojacker makes someone else pay that bill

:::

::: {.fragment style="font-size: 0.95em; margin-top: -0.1em;"}

{{< iconify fa6-solid people-line >}} **Why mine quietly instead of stealing
files? What are the trade-offs of this approach? Now, let's investigate
further!**

:::

## Simulating a Mining Loop

```{python}
import hashlib

def mine(zeros, limit=500_000):
    target = "0" * zeros
    for nonce in range(limit):
        block = f"block-{nonce}".encode()
...
- `zeros` sets how many leading zeros count as a win

- `hashlib.sha256` hashes one candidate block

- `digest.startswith(target)` tests each guess

- The `for` loop tries each `nonce`, up to `limit` guesses

:::

## Measuring the Cost of Mining
...
for zeros in (3, 4):
    ...
    nonce, digest = mine(zeros)
    elapsed = time.perf_counter() - start
    print(f"{zeros} zeros: {nonce + 1:,} guesses, "
          f"{elapsed * 1000:.1f} ms, {digest[:12]}...")
```

::: {.fragment style="margin-top: 0.00em; font-size: 0.85em;"}

- With three hexadecimal leading zeros, a guess wins about once in 4,096

- At four zeros a win takes 65,536 guesses, which is much more!

- The runs above are single, lucky draws with an illustrative model

- Don't forget, mining costs are paid by the computer's owner!
...
- Examples of common installation methods:
  ...
  - Generic pattern: `/bin/bash -c "$(curl -fsSL https://example.invalid/install.sh)"`
...
- What are common ways to deliver malware?
  - Spear phishing
  - Whaling
  - Smishing
  - Vishing
...
::: {.fragment style="font-size: 0.95em; margin-top: -0.4em;"}

**{{< iconify ph scales >}} *Trade-offs*: Attackers trade off ease of use and
effectiveness; users trade off convenience and security. Leads to problems!**
...
- **Important reminders**:
  ...
  - Use a dedicated, isolated virtual machine
  - Do not run unknown samples on a host system
...
- Common indicators:
  - Hash of a known malicious file or object code
  - IPs, URLs, or domains of command-and-control servers
  - Strings or byte patterns in suspicious code
...
- Can reveal readable text embedded in a file
...
- Obscures a malware's source code or binary

- Can make static analysis harder, but does not prevent detection
...
or C program? How hard do you think it is to reverse engineer? Why?**
...
```
::: {.fragment style="margin-top: 0.75em; font-size: 0.95em;"}

{{< iconify fa6-solid people-line >}} **This snapshot shows some of the files and directories
stored in the computer's filesystem. What do you notice? Why is this possibly sensitive? How
could an attacker exploit this data?**
...
- System configuration data may be sensitive:
  ...
  - May contain access credentials or tokens

:::

::: {.fragment .fade .boxed-content style="padding-top: .10em; padding-bottom: .25em; font-size: 0.8em;"}
...
- `strings` analysis may be less effective, but it is still possible
- {{< iconify fa6-solid lightbulb >}} **What are the benefits of obfuscating malware?**
...
- {{< iconify fa6-solid lightbulb >}} **What are the downsides of obfuscating malware?**
...
- Steps in the malware lifecycle:
  ...
  - Installation
  - Command and Control
  - Actions on Objectives (such as exfiltration)

:::

## Case Study: Emotet Trojan

::: {.fragment style="margin-top: 0.00em; font-size: 0.95em;"}

- Malware targeting Windows systems
  - First observed in 2014
  - Historically spread through malicious email attachments
  - Later served as a platform for delivering other malware, including
  ransomware, often with consequences for victims

:::

::: {.fragment style="margin-top: -0.5em; font-size: 0.95em;"}
...
## Malware and Objectives

::: {.columns style="margin-top: 0.15em;"}

::: {.column style="width: 33%; font-size: 0.72em;"}
...
::: {.fragment .fade .boxed-content style="margin-top: -0.25em; font-size: 0.72em;"}

- **CSP Chapter 3 connection**: connect malware **types** and **targets** to
**delivery**, **obfuscation**, and **defense**
...
- **CS-403-3**: Correctly use hardware and/or software **tools** to diagnose
and fix web and **network security risks**.
