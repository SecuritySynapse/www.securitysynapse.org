...
# What is a technology practice that you need to rethink from the perspective of computer security? Why?
...
# *Admonition*: avoid the temptation to "roll your own" cryptographic algorithms! Why?
...
Ways to balance security, privacy, and performance? Best approach?
...
- Insights about the RSA algorithm:
  ...
  - Encryption is "easy" and breaking decryption is "hard"
...
- **Historical Examples**: SHA-256, SHA-512, and MD5
- Insights about the SHA-256 algorithm:
  - Fixed length output
  - Deterministic
  - Fast computation
  - Designed by NSA
  - Cannot generate the input from the output!
...
- {{< iconify fa6-solid lightbulb >}} **Important Insights**:
  ...
  - Yet, only as secure as the weakest link in the system!
...
- **Goal**: Always encrypt HTTP traffic sent to a web site!
- **Problem**: How to make HTTPS more accessible?
- **Solution**: Automated certificate authority system

:::

::: {.fragment style="margin-top: -0.75em; font-size: 0.8em;"}

- Insights about the Let's Encrypt system:
  - Uses Automated Certificate Management Environment (ACME)
  - Provides free SSL/TLS certificates
  - Automates renewal of 90-day certificates
  - Short lifetimes limit leaked-key damage
  - Lifetimes shrinking industry-wide to 45-day
  - Requires a web server to support HTTPS

:::

## Let's Encrypt is Free and Open Source

::: {.incremental style="margin-top: -0.5em; font-size: 0.925em;"}

- How do you get a certificate for your web site?
  - Run an ACME client on the web server
  - Prove domain control: complete a DNS or HTTP challenge
  - Let's Encrypt validates from multiple network perspectives
  - Submit a CSR signed with the authorized account key
  - Let's Encrypt verifies both signatures
  - Issues the certificate and logs it to Certificate Transparency
  - Netlify and similar hosts automate install and renewal
...
## Recap on Content Hashing

::: {.incremental style="margin-top: -0.25em; font-size: 0.95em;"}

- **Key Benefit**: hashing detects tampering and verifies integrity
- **Important Reminder**: hashing is *not* encryption; it is not designed to be
...
- **Review Questions**:
  ...
  - What happens when one character of the input changes?
  - Why is it impossible to recover the input from the hash?
  - How is hashing different from encryption?
  - How is hashing similar to encryption?
...
- **Review Questions**:
  ...
  - What if the `rsa` keys do not match?
...
- **Sign**: use the private key to sign a message hash
- **Verify**: use the public key to check the signature
- **Non-repudiation**: a sender cannot deny a signed message
- Certificate authorities sign Let's Encrypt certificates
- Only the private key signs; the public key verifies

:::

::: {.fragment .fade .boxed-content style="margin-top: -0.5em; font-size: 0.75em;"}

- **Key Questions**:
  - Who can create a digital signature? Who can verify one?
  - Why can a sender not deny a signed message?
  - How do signatures provide *authenticity* and *non-repudiation*?
  - How do signed certificates support the Let's Encrypt case study?
...
- **Chapter 2 connection**: make sure that you understand the key terms and mathematical
  foundations of cryptography and hashing
