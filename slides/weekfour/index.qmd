...
::: {.fragment .fade style="margin-top: 0.00em; font-size: 0.8em;"}
...
for symbol in message:
    if symbol in SYMBOLS:
        ...
        # handle wraparound with modulo, supporting any key size
        if mode == 'encrypt':
...
- **Review Questions**:
  ...
  - Can you explain the update to `translated`?
...
- **Review Questions**:
  ...
  - Why does the algorithm need to handle "wraparound"?
  - Can you explain `symbolIndex = SYMBOLS.find(symbol)`?
...
- **Key Insight**: assumes knowledge of the shared secret key
- **Important Reminder**: decryption differs from encryption
...
- **Review Questions**:
  ...
  - Why `int(math.ceil(len(message) / float(key)))`?
...
- Run for a range of keys, which set the number of columns
...
flowchart LR
  ...
  linkStyle 0,1,2,3 stroke:#1c1c1c,color:#1c1c1c,stroke-width:3px
```
...
flowchart LR
  ...
  linkStyle 0,1,2,3,4,5,6,7,8 stroke:#1c1c1c,color:#1c1c1c,stroke-width:3px
```
...
def encrypt_string(input_string: str, key: bytes):
...
def decrypt_string(encrypted_data: bytes, key: bytes):
...
def test_encryption_decryption(message):
    ...
    assert decrypted_data == message, \
        'Decryption does not match original message'
...
for i in range(5):
    ...
    print(f"Test {i + 1}: passed for length {length}.")
```

::: {.fragment .fade style="margin-top: 0.00em; font-size: 0.9em;"}

- Seeded `random` makes the five tests repeatable
- Each test asserts decryption matches the original
- Summarize general strategy for correctness testing of ciphers?
...
- **Key Insight**: establish confidence in cipher correctness
...
- **Review Questions**:
  ...
  - How does `test_encryption_decryption` work?
...
- How can we hack the transposition cipher?
  - Make these assumptions:
    ...
    - The language in use is not clear
...
def perform_transposition_hack():
    myMessage = (
        "AaKoosoeDe5 b5sn ma reno ora'lhlrrceey e  enlh na  indeit n uhoretrm"
        " au ieu v er Ne2 gmanw,forwnlbsya apor tE.no euarisfatt  e mealefedh"
        "sppmgAnlnoe(c -or)alat r lw o eb  nglom,Ain one dtes ilhetcdba. t tg"
        " eturmudg,tfl1e1 v  nitiaicynhrCsaemie-sp ncgHt nie cetrgmnoa yc r,i"
        "eaa  toesa- e a0m82e1w shcnth  ekh gaecnpeutaaieetgn iodhso d ro hAe"
        " snrsfcegrt NCsLc b17m8aEheideikfr aBercaeu thllnrshicwsg etriebruai"
        "sss  d iorr."
    )
...
for i in range(5):
    ...
    print(f"Test {i + 1}: length {length}, "
          f"ET: {etime:.4f}ms, DT: {dtime:.4f}ms.")
```

::: {.fragment .fade style="margin-top: 0.00em; font-size: 0.8em;"}

- Seeded lengths keep the five runs comparable
- Reports encryption and decryption time in milliseconds
- Each run asserts the round-trip is correct

:::

## Recapping Key Trade-offs

::: incremental

- **Symmetric keys**: one shared secret, fast bulk encryption
- **Asymmetric keys**: no shared secret, slower exchanges
- **Key length**: longer keys resist brute-force attacks
- **Distribution**: exchange keys openly, share secrets safely
- **Evidence**: `Fernet` encrypts and decrypts in milliseconds
- **Balance**: match the key type to the threat and budget

:::

::: {.fragment .fade .boxed-content style="margin-top: -0.25em; font-size: 0.9em;"}

**Insight**: symmetric for bulk data, asymmetric for key exchange. Questions on
trade-offs between symmetric and asymmetric keys?
...
## Cryptography and Objectives

::: {.columns style="margin-top: 0.15em;"}

::: {.column style="width: 33%; font-size: 0.75em;"}
...
- **CCP Chapters 4--12 connection**: illustrate key concepts by building,
testing, and hacking the reverse, Caesar, and transposition ciphers, and
`Fernet` speed study

- **CS-403-1**: Correctly identify the **threats** to an application's security
and describe the **suitable mitigation(s)** for addressing the threats.
